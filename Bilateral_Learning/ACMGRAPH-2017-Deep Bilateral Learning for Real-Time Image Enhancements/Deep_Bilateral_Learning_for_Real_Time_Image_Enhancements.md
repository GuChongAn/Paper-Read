...
文章：Deep Bilateral Learning for Real-Time Image Enhancement

作者：Gharbi, Michaël、Chen, Jiawen、Barron, Jonathan T.et al.
...
class HDRPointwiseNN(nn.Module):
    ...
    def forward(self, lowres, fullres):
        coeffs = self.coeffs(lowres) # 计算双边网格的系数
        guide = self.guide(fullres) # 计算引导图
        slice_coeffs = self.slice(coeffs, guide) # slice层
        out = self.apply_coeffs(slice_coeffs, fullres)
        return out
```

- 模型分为了四个子网
  - Coeffs，对应图中的黄色背景部分，通过低像素的输入图像，计算双边网格仿射系数
  - GuideNN，对应图中的紫色部分，通过原始输入图像，计算同样大小的引导图
  - Slice，对应图中的红色部分，Slice操作
  - ApplyCoeffs，对应图中的绿色部分，计算最后的输出图像
...
关键在于grid_sample的理解，其实就是使用gird提供坐标，在input中查找元素，对找到的元素坐标作插值，得到最后的输出

参考[grid_sample()函数及双线性采样](https://zhuanlan.zhihu.com/p/112030273)
